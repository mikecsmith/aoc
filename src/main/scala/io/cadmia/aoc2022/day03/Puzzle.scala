...
lazy val LookupTable =
  (('a' to 'z') ++ ('A' to 'Z')).zipWithIndex.map(t => (t(0), t(1) + 1)).toMap

// Data Structures
case class BadgeState(
    prioritySum: Int = 0,
    currentElf: Int = 0,
    checkedRucksacks: Rucksack = UncheckedRucksack
)

// Shared utils
def getPriorityValue(char: Char): Int = {
  val GetPriorityError = () =>
    throw new Exception("Unable to retrieve priority")
...
lazy val getDuplicatePriorityForLine =
  makeRucksackCompartments.andThen(getPriority)
...
def processBadgeLine(badgeState: BadgeState, line: String): BadgeState = {
  ...
  val updatedRucksacks = badgeState.checkedRucksacks.updated(
    badgeState.currentElf,
    newRucksackContents
  )
  badgeState.currentElf match
    case a if LastElf == a =>
      BadgeState(
        badgeState.prioritySum + getPriority(updatedRucksacks),
        0,
        UncheckedRucksack
      )
    case b if 0 until LastElf contains b =>
      BadgeState(
        badgeState.prioritySum,
        increment(badgeState.currentElf),
        updatedRucksacks
      )
...
@main def solvePuzzle(): Unit = {
  val (duplicatePrioritySum, badgeState) = Source
    .fromResource("aoc/2022/day03/input")
    .getLines
    .foldLeft(0, BadgeState())((acc, line) => {
      val (duplicatePrioritySum, badgeState) = acc
      val newDuplicatePrioritySum =
        duplicatePrioritySum + getDuplicatePriorityForLine(line)
